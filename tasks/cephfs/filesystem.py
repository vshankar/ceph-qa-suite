...
class Filesystem(MDSCluster):
    ...
    def getinfo(self):
        ...
        self.get_pool_names(status = status)
    ...
    def get_pool_names(self, refresh = False, status = None):
        if refresh or self.metadata_pool_name is None:
            if status is None:
                status = self.status()
            fsmap = status.get_fsmap(self.id)
